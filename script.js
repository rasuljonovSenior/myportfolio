...
form.addEventListener("submit", async (e) => {
  ...
  formBtn.setAttribute("disabled", "");
  formBtn.innerHTML =
    '<ion-icon name="hourglass-outline"></ion-icon><span>Yuborilmoqda...</span>';
...
📅 Vaqt: ${new Date().toLocaleString("uz-UZ")}
  ...
  try {
    ...
    if (response.ok && result.ok) {
      ...
      // Reset button
      formBtn.innerHTML =
        '<ion-icon name="paper-plane"></ion-icon><span>Xabar Yuborish</span>';
      formBtn.setAttribute("disabled", "");
      ...
      throw new Error(
        result.description || "Xabar yuborishda xatolik yuz berdi"
      );
    }
  } catch (error) {
...
class ShellGame {
  ...
  startGame() {
    ...
    this.messageEl.textContent =
      "🔴 Qizil koptokni diqqat bilan kuzatib turing!";
